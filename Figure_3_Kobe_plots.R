...
Overfished_overfishging <- nrow(subset(kobe_data,F_Fmsy >1 & S_Smsy < 0.8))/nrow(kobe_data)
Overfished_underfishging <- nrow(subset(kobe_data,F_Fmsy <1 & S_Smsy < 0.8))/nrow(kobe_data)
Underfished_overfishging <- nrow(subset(kobe_data,F_Fmsy >1 & S_Smsy > 1.2))/nrow(kobe_data)
Underfished_underfishging <- nrow(subset(kobe_data,F_Fmsy <1 & S_Smsy > 1.2))/nrow(kobe_data)
Overfishging <- nrow(subset(kobe_data,F_Fmsy >1 ))/nrow(kobe_data)
Overfished <-   nrow(subset(kobe_data,S_Smsy < 0.8))/nrow(kobe_data)
...
p3 <- ggplot() + 
  geom_density(data=full.sims.top, aes(x=F_Fmsy, group=control,fill=control),alpha=0.65, adjust=2.5)+ 
...
p7 <- ggplot() + 
  geom_density(data=btml, aes(x=S_Smsy, group=risk,fill=risk),alpha=0.65, adjust=1.5)+ 
...
p8 <- ggplot() + 
  geom_density(data=btmr, aes(x=S_Smsy, group=risk,fill=risk),alpha=0.65, adjust=3)+ 
...
jpeg("figures/fig_3_kobe_3sims_phi_0.8.Rho_0.6.logFE_0.1.OU_0.Feb152019.jpeg",width=6, height=6, units="in",res=800)
