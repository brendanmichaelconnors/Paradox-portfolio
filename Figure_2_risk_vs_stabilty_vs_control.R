...
jpeg("figures/fig_2_slices_control_phi_0.8.Rho_0.6.logFE_0.27.OU_0.jpeg",width=4, height=4, units="in",res=800)
...
sims_out <- subset(long_sims, control == 0)
...
sims_out <- subset(long_sims, control == 0.12)
x <- as.numeric(paste(sims_out[,1]))
y <- as.numeric(paste(sims_out[,2]))
z <-as.numeric(sims_out[,3]*100)
data.loess= loess(z~x*y)
...
sims_out <- subset(long_sims, control == 0.25)
x <- as.numeric(paste(sims_out[,1]))
y <- as.numeric(paste(sims_out[,2]))
z <-as.numeric(sims_out[,3]*100)
data.loess= loess(z~x*y)
...
sims_out <- subset(long_sims, control == 0.5)
x <- as.numeric(paste(sims_out[,1]))
y <- as.numeric(paste(sims_out[,2]))
z <-as.numeric(sims_out[,3]*100)
data.loess= loess(z~x*y)
...
sims_out <- subset(long_sims, control == 0.75)
x <- as.numeric(paste(sims_out[,1]))
y <- as.numeric(paste(sims_out[,2]))
z <-as.numeric(sims_out[,3]*100)
data.loess= loess(z~x*y)
