##=============================================================================
## Making panel plot for case study system
##=============================================================================

# This script makes the panel plot, that can then be combined with the map to
# make the final figure in the manuscript for the case studies
...
# -----------------------------------------------------------------------------
# Create one list() that contains the information from all three systems
# -----------------------------------------------------------------------------
...
for(j in 1:length(systems)) {
  temp.S <- read.csv(paste(systems[j], "/Outputs/", systems[j], "_Smsy_", yr.range, "yrs.csv", 
                           sep=""), header=T)
  colnames(temp.S) <- c("lake", "meanSmsy", "SlowerCI", "SupperCI",
                        "S1970s", "S1980s", "S1990s", "S2000s", "SAll")
  temp.U <- read.csv(paste(systems[j], "/Outputs/", systems[j], "_Umsy_", yr.range, "yrs.csv", 
                            sep=""), header=T)
